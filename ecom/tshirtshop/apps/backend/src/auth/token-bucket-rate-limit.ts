...
function consumeToken(
  ...
  const refilled = Math.min(
    capacity,
    current.tokens + elapsedSeconds * refillTokensPerSecond,
  );
  ...
  if (refilled < 1) {
    ...
    const retryAfterSeconds = Math.max(
      1,
      Math.ceil(missingTokens / refillTokensPerSecond),
    );
    store.set(key, {
      tokens: refilled,
      lastRefillAtMs: nowMs,
...
export function createTokenBucketRateLimitMiddleware(
  ...
  resolvePath: (req: Request) => string = (req) =>
