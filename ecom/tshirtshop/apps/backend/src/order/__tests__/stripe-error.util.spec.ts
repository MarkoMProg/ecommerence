...
describe('mapStripeError', () => {
  ...
  it('maps card_declined to CARD_DECLINED', () => {
    const result = mapStripeError({ code: 'card_declined', type: 'card_error', message: 'Declined' });
    expect(result).toEqual({
      code: 'CARD_DECLINED',
      userMessage: 'Your card was declined. Please try another card or contact your bank.',
    });
  });

  it('maps insufficient_funds to INSUFFICIENT_FUNDS', () => {
    const result = mapStripeError({ code: 'insufficient_funds', message: 'Insufficient' });
  ...
  it('maps api_connection_error to GATEWAY_TIMEOUT', () => {
    const result = mapStripeError({ type: 'api_connection_error', message: 'Network' });
  ...
  it('maps rate_limit_error to GATEWAY_ERROR', () => {
    const result = mapStripeError({ type: 'rate_limit_error', message: 'Too many' });
