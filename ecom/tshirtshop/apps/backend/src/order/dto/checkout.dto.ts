...
export function validateShippingAddress(addr: unknown): ValidationError[] {
  ...
  if (!line1) {
    ...
    if (containsHtml(line1)) {
      errors.push({
        field: 'shippingAddress.line1',
        message: 'line1 must not contain HTML',
      });
    }
  }
  ...
  if (!city) {
    ...
    if (containsHtml(city)) {
      errors.push({
        field: 'shippingAddress.city',
        message: 'city must not contain HTML',
      });
    }
  }
  ...
  if (!stateOrProvince) {
    errors.push({
      field: 'shippingAddress.stateOrProvince',
      message: 'stateOrProvince is required',
    });
  } else {
    if (stateOrProvince.length > MAX_LENGTHS.stateOrProvince) {
      errors.push({
        field: 'shippingAddress.stateOrProvince',
        message: `stateOrProvince must not exceed ${MAX_LENGTHS.stateOrProvince} characters`,
      });
    }
  }
  ...
  if (phone) {
    if (phone.length > MAX_LENGTHS.phone) {
      errors.push({
        ...
        message: `phone must not exceed ${MAX_LENGTHS.phone} characters`,
      });
